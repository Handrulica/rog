...
export const StepperIndicator = React.forwardRef<HTMLImageElement, StepperIndicatorProps>(
...
export const stepperLineVariants = cva(
  'w-[4px] md:w-[3px] mx-auto',
  {
    variants: {
      variant: {
        default: 'bg-green-30',
...
export interface StepperLineProps extends React.HTMLAttributes<HTMLDivElement>, VariantProps<typeof stepperLineVariants> { };

export const StepperLine = React.forwardRef<HTMLDivElement, StepperLineProps>(
  ({ variant, className, ...props }, ref) => {
    return (
      <div ref={ref} className={cn(stepperLineVariants({ variant, className }))} {...props} />
    );
  }
);

export const stepperTitleVariants = cva(
  'heading-md sm:ml-8 lg:heading-md',
  {
    variants: {
      variant: {
        default: 'text-green-30 sm:text-primary-100 sm:heading-xs sm:font-bold',
        blue: 'text-blue-80',
        disabled: 'text-neutrals-100 sm:text-primary-100 sm:heading-xs sm:font-bold ml-6 sm:ml-14'
      }
    },
    defaultVariants: {
      variant: 'default'
...
export interface StepperTitleProps extends React.HTMLAttributes<HTMLDivElement>, VariantProps<typeof stepperTitleVariants> {
  children?: string,
};
...
export const StepperTitle = React.forwardRef<HTMLDivElement, StepperTitleProps>(
  ({ variant, className, children, ...props }, ref) => {
    return (
        <h2 ref={ref} className={cn(stepperTitleVariants({variant, className}))} {...props}>
          {children}
        </h2>
    );
  }
